...
if (check_auth('scpl_add')) { ?>
	...
	<div style="margin:5px 0px;">
		<button onClick="document.getElementById('jyts_add').style.display=''">发布市场评论</button>
	</div>
<?php
}
if (check_auth('scpl_view')) { ?>
	<div
		style=" padding:20px; margin-bottom:10px;border:1px solid #CCC; <?php if ($act != 'scpl_view') echo 'display:none'; ?>">
		<div style="font-size:20px; line-height:25px; text-align:center"><strong><?= $row['title'] ?></strong></div>
		<div style="text-align:center"><?= $row['dj'] ?>
			星 <?= date('Y-m-d H:i:s', $row['atime']) ?> <?= $row['user'] ?></div>
