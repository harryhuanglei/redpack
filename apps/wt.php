...
if (check_auth('wt_re') && $act == "wt_view") { ?>
	<form action="?act=wt_re&id=<?= $row[id] ?>" method="post" enctype="application/x-www-form-urlencoded">
		<table width="100%" cellspacing="0" id="mytable"
			   style="border-left: 1px solid #C1DAD7;border-top: 1px solid #C1DAD7; margin-bottom:5px;">
			<tr>
				<td width="30" class="tableleft" style="width:80px;">问题：</td>
				<td colspan="3"><?= $row[q] ?><br>
					<script>document.write(ftime(<?=$row[q]?>)); </script>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td width="100"><input name="auser" type="hidden" id="auser"
									   value="<?= $_SESSION['login_user'] ?>"><?= $_SESSION['login_user'] ?></td>
			</tr>
			<tr>
				<td class="tableleft">&nbsp;</td>
				<td colspan="3"><input type="submit" name="button" id="button" class="btn2" value="回答问题"></td>
			</tr>
		</table>
	</form>
<?php
...
if (check_auth('wt_add')) { ?>
	<form action="?act=wt_add" method="post" enctype="application/x-www-form-urlencoded" onSubmit="return docheck();">
		<table width="100%" cellspacing="0" id="wt_add"
	...
	<div style="margin:5px 0px;">
		<button class="btn1" onClick="document.getElementById('wt_add').style.display=''">提出问题</button>
	</div>
<?php
} else {
