...
if (check_auth('jyts_add')) { ?>
	<form action="?act=jyts_add" method="post" enctype="application/x-www-form-urlencoded">
		<table width="100%" cellspacing="0" id="jyts_add"
			...
			<tr>
				...
				<td><textarea name="txt" id="txt" style="width:100%"
							  class="xheditor {cleanPaste:0,height:'300',internalScript:true,inlineScript:true,linkTag:true,upLinkUrl:'../upload/upload.php',upImgUrl:'../upload/upload.php',upFlashUrl:'../upload/upload.php',upMediaUrl:'../upload/upload.php'}"></textarea>
				</td>
			</tr>
			<tr>
	...
	<div style="margin:5px 0px;">
		<button onClick="document.getElementById('jyts_add').style.display=''">发布交易提示</button>
	</div>
<?php
}
if (check_auth('jyts_view')) { ?>
	<div
		style=" padding:20px; margin-bottom:10px;border:1px solid #CCC; <?php if ($act != 'jyts_view') echo 'display:none'; ?>">
		<div style="font-size:20px; line-height:25px; text-align:center"><strong><?= $row['title'] ?></strong></div>
		<div style="text-align:center"><?= date('Y-m-d H:i:s', $row['atime']) ?> <?= $row['user'] ?></div>
...
<?php
	...
	echo "<script>top.layer.msg('没有权限查看交易提示！请联系客服！');</script>";
} ?>
