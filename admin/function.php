<?php
//if(!isset($_SESSION['admincp']))exit("http://".$_SERVER["HTTP_HOST"]."/admin/login.php");
if(!isset($_SESSION['login_uid']) || empty($_SESSION['login_uid']))exit("http://".$_SERVER["HTTP_HOST"]."/minilogin1.php");
 ...
 * @param $id
 * @param $store
 * @param $store_act_type
 * @param $edit_store
 * @param $desc
 */
function user_store($id, $store, $store_act_type, $edit_store, $desc){
    global $db,$tablepre;

//加减用户积分
    $edit_store = $store_act_type ? $edit_store : 0 - $edit_store;

    if (($edit_store + $store) < 0) {
        exit("<script>alert('用户剩余积分不足！！');location.href='user_store_edit.php?id=$id';</script>");
    }
    if (!$desc) {
//        exit("<script>alert('请填写备注说明！！');location.href='user_store_edit.php?id=$id';</script>");
        $desc = '管理员添加';
    }
    $state_id=$db->query("select user_id from {$tablepre}user_store where user_id='$id' limit 1");
    $time=time();
    $stores=$edit_store + $store;
    $row = $db->fetch_row($state_id);
//    exit("<script>alert($sql);location.href='user_store_edit.php?id=$id';</script>");
    if ($row) {
        $db->query("update {$tablepre}user_store set store='$stores' where user_id='$id'");
        $db->query("insert into {$tablepre}user_store_flow(`user_id`,`store_change`,`desc`,`c_time`)values('$id','$edit_store','$desc','$time')");
    }else{
        $db->query("insert into {$tablepre}user_store(`user_id`,`store`,`c_time`)values('$id','$stores','$time')");
        $db->query("insert into {$tablepre}user_store_flow(`user_id`,`store_change`,`desc`,`c_time`,`status`)values('$id','$edit_store','$desc','$time','1')");
    }
//
 ...
 * @return bool
 */
function open_num($num1,$num2,$num3,$num4,$num5){
	global $db,$tablepre;
//	if($password!="")$pwd=" ,password='".md5($password)."',";
//	else $pwd=',';
//	if(stripos(auth_group($_SESSION['login_gid']),'users_group')!==false)
...
//	$db->query("update {$tablepre}members set regdate='".time()."',username='$username' , realname='$realname' $pwd phone='$phone',money=$money,fuser='$fuser',tuser='$tuser',state='$state' where uid='$uid'");
    ...
    $query = $db->query("select turn_id,is_end from {$tablepre}turn where `type` = 1 order by turn_id desc limit 1");
    $rst = $db->fetch_row($query);
    if ($rst['is_end'] == 0){
        //当前下注未结束
        exit("<script>alert('当前下注未结束！！');location.href='account.php';</script>");
        return false;
    }
    $turn_id = $rst['turn_id'];
    $check_query = "select turn_id from {$tablepre}user_accouts_nums where `turn_id` =  $turn_id  limit 1";
    $check_rst = $db->fetch_row($db->query($check_query));//判断是否已经存在
    if ($check_rst){
        exit("<script>alert('本局已经输入过了！！');location.href='account.php';</script>");
        return false;//本局已经输入过了
    }
    $time = date('Y-m-d H:i:s');
    $db->query("insert {$tablepre}user_accouts_nums  (n1,n2,n3,n4,n5,turn_id,c_time) values('$num1','$num2','$num3','$num4','$num5','$turn_id','".$time."')");
}
...
{
    exit("<script>alert('{$msg}');top.location.href='{$url}';</script>");
}
 ...
 * @param $chip_integral
 */
function chip_add($chat_id, $chip_name, $chip_integral){
    global $db,$tablepre;
    $sql="insert into {$tablepre}chip (`chat_id`, `chip_name`, `chip_integral`) values ('{$chat_id}', '{$chip_name}', '{$chip_integral}')";
    $db->query($sql);
    return true;
}
 ...
 * 清理当下注记录
 */
function clear_day_act()
{
    global $db,$tablepre;
    $sql = "truncate chat_turn";
    $sql1 = "truncate chat_user_accouts_nums";
    $sql2 = "truncate chat_user_store_flow";
    $sql3 = "truncate chat_user_chip";
    $db->query($sql);
    $db->query($sql1);
    $db->query($sql2);
    $db->query($sql3);
