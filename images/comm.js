...
function msgubbface(){
	...
	var p='';
	for(var i=0;i<105;i++){
		p+='<div style="float:left"><img src="face/pic/'+i+'.gif" onclick="$(\'txt\').value+=\':em'+i+':\';"/></div>';
		}
	facediv.innerHTML=p;
	}
...
function writeUbb(str){
	re=/:em(\d{1,3}):/gi;  
	str=str.replace(re,"<img   src='face/pic/$1.gif'   border='0'>");
	document.write(str);
}
...
document.write('<span onClick="closeWithIframe()" style="float:right; cursor:pointer;padding:5px;" class="titlesss"><img src="images/close.gif"></span>');
